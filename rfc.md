...
| `getVendors`, `getPurposes`, `hide`, `show`, `getHistory`, `listRegistrations`, `registerAssistant` | MUST be rejected | MUST be allowed |
...
- Browser MUST attach runtime provenance metadata (`topLevelOrigin`, `frameOrigin`, `scriptOrigin`) to history entries and related event payload metadata.
...
8. Implementations MUST enforce anti-spam controls including registration quotas, payload caps, mutation rate limits, and bounded operational history retention.
...
type ConsentEventType = "update" | "hide" | "show" | "withdraw" | "query" | "registerassistant" | "regulation_change" | "consent_request";
...
- Re-registration with the same `cmp.id` MAY update existing metadata and SHOULD keep audit continuity.
- Browser MUST append a history entry for each registration and registration update.
...
- `payload.cmp` carries structured CMP identity: `version` (CMP product version, required), and optional `id`, `sdkVersion`, `configVersion`, `language` (BCP 47), `displayName`, and `frameworks` (for example `tcf-v2.2`, `gpc`, `gpp`).
- `payload.catalogChecksum` MAY be supplied as an opaque CMP-computed digest covering the vendor and purpose catalog. The browser MUST store the value verbatim alongside the registration and MUST NOT interpret, hash, or validate it. Storage scope is per origin per `cmp.id`; where `cmp.id` is absent, scope is per origin per `vendor`.
- The previously-stored checksum (or `null` if none) is surfaced to the CMP through the registration return shape so the CMP can decide whether to skip subsequent `registerVendors()` and `registerPurposes()` calls.
...
- CMPs MAY skip this call when the `storedCatalogChecksum` returned by `registerInterface()` matches the locally computed `catalogChecksum`.
...
#### 6.5.7 `navigator.consent.getHistory()`
...
- Returns the chronological consent history available in the current context and permissions.
- Entries cover preference updates, withdrawals, hide/show actions, regulation changes, and assistant or registration lifecycle events.
- Browser MUST honor anti-spam controls and bounded response windows defined in Section 10.
- Calls to this method MUST be recorded as history entries of type `query` for traceability.

#### 6.5.8 `navigator.consent.listRegistrations()`
...
- Returns the currently-active interface registrations visible in the calling context as a snapshot, distinct from the chronological view returned by `getHistory()`.
- `filter.cmpId` matches against `InterfaceRegistration.cmp.id`.
- `filter.origin` matches against the registration's top-level origin.
- If no filter is provided, returns all registrations the caller is permitted to see.
- Browser MUST NOT include teardown or stale registrations in the result.

#### 6.5.9 `navigator.consent.registerAssistant()`
...
- Browser MUST append a history entry for each `setRegulations()` call.
...
6. Browser SHOULD include applied and blocked conflict outcomes (with provenance metadata) in history entries.
...
5. Keep bounded operational history retention and bounded `getHistory()` response windows to prevent unbounded runtime growth.
...
2. Retain only necessary history-entry fields for compliance evidence.
...
- Access to history data SHOULD be scoped to authorized contexts and permissions.
...
- Browsers SHOULD expose a dedicated DevTools panel for consent declarations and the consent history timeline.
...
- Implementations MUST provide controls across five categories:
  ...
  4. Bounded operational history retention and bounded `getHistory()` response windows.
  5. Abuse observability via reason-coded warnings with provenance metadata.
- Re-registration with the same `cmp.id` SHOULD update an existing registration instead of consuming a new quota slot.
...
3. Updated snapshot and history entry are available.
